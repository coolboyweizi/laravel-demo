...
class PublicController extends Controller
{
    use Msg;
    //图片上传处理
    public function uploadImg(Request $request, StorageService $storageService)
    {
        $key = $storageService->put(
            $request->file('file'),
            'demo',
            []
        );
...
/*        $disk = QiniuStorage::disk('qiniu');
        ...
        try {
            $key = $storageService->put($file,Carbon::now()->format('Y-m-d'),['name'=>$newFile]);
            return [
                'code'  => 0,
                'msg'   => '上传成功',
                'data'  => [
                    'src' => $storageService->get($key),
                    'title' => $key
                ],
            ];
